...
var estoqueBaixo = listaProdutos.Where(p => p.Preco < 15).OrderBy(p => p.Nome);
...
var resultado = listaProdutos.Where(p => p.Preco < 500).OrderBy(p => p.Nome).Select(p => new { nomeProduto = p.Nome.ToUpper(), precoModificado = p.Preco * 1.1 });
...
var resultado2 = listaProdutos.Where(p => p.Preco > 200).OrderBy(p=>p.Preco).Select(p => new { nomeProduto = p.Nome.ToUpper(), precoModificado = p.Preco * 0.8 });
